...
function Init(){
	...
	helplighter = new THREE.PointLight({color:new THREE.Color(0xffffff) });
	//scene.add(helplighter);
	camera.add(helplighter);
...
//scene.add(curveObject);
	...
	let yOffset = 0;
	let xx = 0;
	let texture = new THREE.TextureLoader().load("textures/Frame1.jpg", function(txt){
		...
		txt.flipY = false;
		let specMaterial = new THREE.MeshStandardMaterial({ side: THREE.DoubleSide, wireframe:false, transparent:true, map:txt})
		for (let i=0;i<70;i+=7){
			xx+=1.5
			let xOffset = 0;
			for(let j=-100;j<200;j+=14){
				
				let bufferGeometry = new THREE.PlaneBufferGeometry(1.5, 14, 1);
				bufferGeometry.attributes.uv.array[0] = parseFloat((xOffset).toFixed(2),10) //x0y1 
				bufferGeometry.attributes.uv.array[1] = parseFloat((yOffset).toFixed(2),10)
				
				bufferGeometry.attributes.uv.array[2] = parseFloat((xOffset+(1/22)).toFixed(2),10) //x1y1
				bufferGeometry.attributes.uv.array[3] = parseFloat((yOffset).toFixed(2),10) 
	
				bufferGeometry.attributes.uv.array[4] = parseFloat((xOffset).toFixed(2),10) //x0y0
				bufferGeometry.attributes.uv.array[5] = parseFloat((yOffset+0.11).toFixed(2),10)
	
				bufferGeometry.attributes.uv.array[6] = parseFloat((xOffset+(1/22)).toFixed(2),10) //x0y1
				bufferGeometry.attributes.uv.array[7] = parseFloat((yOffset+(0.11)).toFixed(2),10)
				
				
	
				let tmp = new THREE.Mesh(bufferGeometry,specMaterial)
				tmp.position.set(xx,j+14,0);
				tmp.amplitude=0.01+Math.random()*(0.01-0.005);
				scene.add(tmp);
				xOffset=parseFloat((xOffset+(1/22)).toFixed(2),10);
				//GroupArray.push(tmp);
				console.log(bufferGeometry.attributes.uv.array)
				
			}
			
			
			yOffset=parseFloat((yOffset+(0.11)).toFixed(2),10);
		}
	})
